...
def fid_diff(a, b, t):
    ...
    bds_ref = BDS(diag_elem=[0.95, 0.05/3, 0.05/3, 0.05/3], pauli_strength=relative_strengths)
    bds_ref.evolve_pauli(t2 - t1)
    ref_fid = bds_ref.get_fid()
    
    fid_diff = fid - ref_fid
...
F = fid_diff(A3, B3, t3)       # ⇒ shape (N_b, N_a, N_t)

# find the index of the max along the t-axis:
fid_diff_max = np.nanmax(np.where(mask[:,:,None], F, -np.inf), axis=2)
...
fid_diff_max_masked = np.ma.array(fid_diff_max, mask=~mask)
...
pcm = ax.pcolormesh(A, B, fid_diff_max_masked, cmap=cmap, vmin=np.min(fid_diff_max_masked), vmax=np.max(fid_diff_max_masked))

# create the colorbar, capture the object
cbar = fig.colorbar(pcm, ax=ax)

# 1) change the colorbar’s axis‐label size
cbar.set_label(r'Fidelity Difference', fontsize=18)
